import { _ as lt, a as fileUpload, c as projectContext, d as desc, f as and, g as inArray, h as gte, i as chat, l as projectFile, m as gt, n as isDatabaseAvailable, o as message, p as eq, r as drizzle, s as project, u as asc, v as sql } from "./connection-pool-Bhce9fke.mjs";
...
async function ensureDb() {
	...
	if (authMethod === "oauth" || authMethod === "cli") {
		...
		try {
			const { getDb } = await import("./connection-pool-CwyWbD1P.mjs");
...
async function saveFileUpload({ id, chatId, userId, filename, contentType, fileSize, storagePath, extractedContent, metadata, volumePath, volumeCatalog, volumeSchema, volumeName, storageType, fileChecksum }) {
	if (!isDatabaseAvailable()) {
...
async function getFileUploadById({ id }) {
	...
	try {
		const [result] = await (await ensureDb()).select().from(fileUpload).where(eq(fileUpload.id, id));
		return result || null;
	} catch (error) {
...
//#endregion
export { updateChatLastContextById as A, getProjectFiles as C, saveChat as D, removeFileFromProject as E, generateUUID as F, ChatSDKError as I, getMessageByErrorCode as L, updateProject as M, updateProjectContext as N, saveFileUpload as O, convertToUIMessages as P, getProjectContexts as S, removeChatFromProject as T, getFileUploadById as _, addChatToProject as a, getMessagesByChatId as b, createProject as c, deleteMessagesByChatIdAfterTimestamp as d, deleteProject as f, getChatsByUserId as g, getChatsByProjectId as h, checkChatAccess as i, updateChatVisiblityById as j, saveMessages as k, deleteChatById as l, getChatById as m, postRequestBodySchema as n, addFileToProject as o, deleteProjectContext as p, StreamCache as r, addProjectContext as s, myProvider as t, deleteFileUpload as u, getFileUploadsByChatId as v, getProjectsByUserId as w, getProjectById as x, getMessageById as y };